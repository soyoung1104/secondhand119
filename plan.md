...
2. **바로 접수하기** → 이름/은행명·계좌번호/주소(주소검색)·상세주소/공동현관 비밀번호/
   전화번호(자동 하이픈) 입력 폼(모달)이 뜨고, "확인"을 누르면 해당 정보가 구글 시트에
   한 행으로 저장됨
...
3. 1행에 헤더 입력: `접수일시 | 이름 | 은행명 | 계좌번호 | 주소 | 상세주소 | 공동현관 비밀번호 | 전화번호`

> **이미 시트를 만들어서 운영 중이라면**: 기존 헤더를 위 순서로 바꾸고, `apps-script/dist/Code.gs`의
> 최신 내용을 Apps Script 편집기에 다시 붙여넣은 뒤 **배포 → 배포 관리 → 기존 배포의 연필(수정)
> 아이콘 → 버전: "새 버전" → 배포**를 눌러야 한다. ("새 배포"를 누르면 URL이 바뀌어 버리므로
> 반드시 기존 배포를 "수정"해서 새 버전으로 갱신할 것.)
    ...
    <div class="modal-content">
      ...
      <form id="applyForm" novalidate>
        ...
        <div class="form-row">
          ...
          <label>
            계좌번호
            <input type="text" name="accountNumber" placeholder="000-0000-0000" required inputmode="numeric" />
          </label>
        </div>
        <label>
          주소
          <div class="address-row">
            <input type="text" name="address" id="addressInput" placeholder="주소 검색 버튼을 눌러주세요" readonly required />
            <button id="addressSearchBtn" class="btn-address" type="button">주소 검색</button>
          </div>
        </label>
        <label>
          상세주소
          <input type="text" name="addressDetail" placeholder="동/호수 등 상세주소" />
        </label>
        <label>
          공동현관 비밀번호
        ...
        <label>
          전화번호
          <input type="tel" name="phone" id="phoneInput" placeholder="010-0000-0000" required
                 inputmode="numeric" pattern="^01[016789]-\d{3,4}-\d{4}$" />
        </label>
        <label class="consent">
...
> **추가 UX 규칙**:
> - `이름`은 `maxlength="10"`으로 10자 제한 (HTML 레벨에서 강제, 서버에서도 `isValidPayload`가
>   길이를 재검증)
> - `주소`는 `readonly` + "주소 검색" 버튼으로만 채워지며, 다음(Daum) 우편번호 API
>   (`//t1.daumcdn.net/mapjsapi/bundle/postcode/prod/postcode.v2.js`)의 `oncomplete` 콜백에서
>   도로명 주소를 채운다. API가 반환하는 값은 `any`이므로 `DaumPostcodeData` 인터페이스로
>   즉시 타입을 좁혀서 사용한다. 동/호수 등 세부 주소는 별도의 `상세주소`(`addressDetail`)
>   필드로 사용자가 직접 입력한다.
> - `전화번호`는 `input` 이벤트마다 숫자만 추출해 `010-1234-5678` 형태로 자동 재포맷한다
>   (`formatPhoneNumber`). 이에 맞춰 `pattern`도 하이픈이 항상 있는 형태로 단순화했다.

### 5.4 `web/privacy.html`
개인정보(이름, 은행명, 계좌번호, 주소·상세주소, 공동현관 비밀번호, 전화번호)를 수집하므로
개인정보보호법상 **처리방침 고지 + 동의**가 필요하다. 특히 계좌번호와 공동현관 비밀번호는
금융/보안 정보에
...
- [ ] "바로 접수하기" → 모달 오픈, 은행명/계좌번호가 한 줄에 나란히 표시되는지 확인
- [ ] 이름에 11자 이상 입력 시 10자에서 더 입력되지 않는지 확인
- [ ] "주소 검색" 버튼 클릭 → 다음 우편번호 팝업이 실제로 뜨는지, 주소 선택 후 `주소` 필드에
      정상적으로 채워지는지 확인 (실기기/실브라우저에서 확인 필요 — 자동화 프리뷰로는
      팝업 iframe 렌더링을 완전히 검증하기 어려움)
- [ ] 전화번호 입력 시 숫자만 입력해도 `010-1234-5678` 형태로 자동으로 하이픈이 붙는지 확인
- [ ] 필드(이름/은행명/계좌번호/주소/전화번호) 미입력 시 브라우저 기본 유효성 메시지 노출
- [ ] 상세주소·공동현관 비밀번호는 비워둔 채로도 제출이 되는지 확인 (선택 항목)
- [ ] 동의 체크 안 하면 제출 차단
- [ ] 정상 제출 후 구글 시트에 새 행이 실제로 추가되는지 확인
      (접수일시/이름/은행명/계좌번호/주소/상세주소/공동현관 비밀번호/전화번호)
...
- [x] 폼 개선 4종 반영: 이름 10자 제한(`maxlength`), 은행명/계좌번호 필드 분리, 다음(Daum)
...
- [x] `apps-script/dist/Code.gs` 내용을 실제 Apps Script 프로젝트에 붙여넣고 웹 앱으로 배포 (§4.3)
      — 배포 ID `AKfycbwmY5eXC-wD_kWSDpbQK2Fsk1OXLm73i18uthY9ygcFnkkcBSou3D8QkKdKoPh5fVoB`,
      액세스 권한 "모든 사용자"로 재배포 후 서버 직접 POST 테스트 및 실제 UI 제출 테스트 모두
      `{"result":"success"}` 확인 완료 (시트에 테스트용 행이 들어가 있으니 삭제 필요)
- [x] 배포된 Apps Script URL을 `web/src/script.ts`의 `SHEET_ENDPOINT`에 반영 후 `npm run build:web` 재실행
- [x] `web/assets/logo.png` 실제 로고 이미지 추가
...
- [x] 확정된 URL(`https://soyoung1104.github.io/secondhand119/`)로
      `tools/generate-qr.ts`의 `SITE_URL` 수정 후 `npm run qr` 실행 → `web/assets/qr.png` 생성 완료
- [ ] **(중요, 아직 미완료)** 폼 필드 스키마가 바뀌어서 이미 배포된 시트/Apps Script도
      함께 갱신해야 은행명·상세주소가 저장된다 (§4.1 참고):
      1. 구글 시트 헤더를 `접수일시 | 이름 | 은행명 | 계좌번호 | 주소 | 상세주소 | 공동현관 비밀번호 | 전화번호`로 변경
      2. Apps Script 편집기에 최신 `apps-script/dist/Code.gs` 내용 다시 붙여넣기
      3. 배포 → 배포 관리 → 기존 배포 수정(연필) → 버전 "새 버전" → 배포 (URL 유지, "새 배포" 금지)
- [ ] §8 테스트 체크리스트 전체 수행
